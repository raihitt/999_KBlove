...
- [x] フェーズ6: Keymap 反映 (Quick-tap-ms, K/L Buttons, Combos)
- **完了**: Conductor思想に基づく即時解除、およびKeyball機に最適化したK/Lマウスボタン配置の反映済み
...
2. マウス操作用コンボとキー配置の最適化 [x]
- 左クリックを `K` (Pos 19)、右クリックを `L` (Pos 20) に配置
- `K+L` でミドルクリック、`J+K` でスクロールレイヤーのコンボを追加し、Keyballの操作性を向上
